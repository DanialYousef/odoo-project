...
class TestMailGroupModeration(TestMailListCommon):
    ...
    def test_moderation_rule_api(self):
        ...
        self.assertEqual(moderation_1.state, 'allow')
        self.assertEqual(moderation_2.state, 'allow', 'Should have write on the existing moderation rule')
        self.assertEqual(moderation_3.state, 'ban', 'Should not have changed moderation of the other group')
        new_moderation = mail_group.moderation_rule_ids.filtered(lambda rule: rule.email == 'bob@test.com')
        self.assertEqual(new_moderation.state, 'allow', 'Should have created the moderation with the right status')
